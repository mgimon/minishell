...
/*   Updated: 2024/11/26 19:24:12 by mgimon-c         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// utils_22.c
int						open_files_section_helper(t_section *section, t_file **tmp, t_heredoc **tmp_hdocs);
void					heredocs_norminetter_two(t_section *section, t_token **tmp, t_token **section_start, int count);
int						heredocs_norminetter_one(t_section *section, t_token **tmp, int s, int *section_index);
int						write_heredocs_norminetter(int *len, char **expanded, t_heredoc **tmp_hdoc);
void					removevar_norminetter(t_section *current, int *total_exports, char ***new_exports, int n);

// utils_23.c
int						psnorminetter(const char **start, int *count, char ***path_parts, int *length);
void					handle_forks(t_section **current, int *prev_fd, int (*pipefd)[2]);
void					exexport_printerr(t_section *current, int n);
void					excd_helper(t_section *current, char **var_pwd, char **var_oldpwd, char ***new_env);
void					excd_helper_two(t_section *current, char **var_pwd, char **var_oldpwd);

// utils_24.c
int						rembackdirs_hlpr(t_section *current, int dirs_back, int *i, char ***split_pwd);
...
void					rembackdirs_hlpr_two(t_section *current, int i, char ***new_env, char **new_pwd);

// utils_25.c
void    expandvar_hlp_one(t_general *info, char **exit_status_str, char (*temp)[1024], int *temp_index);
