...
void	remove_var_from_exports(t_section *current, char *var_equal)
{
	...
	i = 0;
	n = -1;
	if (!current || !current->cmdv
		|| !current->info->exports || !current->cmdv[1])
		return ;
	while (current->info->exports[i])
	{
		linedeclare = ft_strjoin("declare -x ", var_equal);
		if (ft_strncmp_pipex(linedeclare,
				current->info->exports[i], ft_strlen(linedeclare)) == 0)
			n = i;
		free(linedeclare);
		i++;
	}
