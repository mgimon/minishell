...
/*   Updated: 2024/11/09 21:18:37 by mgimon-c         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
    ...
    if (pwd && strstr(pwd, "..")) {
        ...
        if (new_pwd) {
            ...
            if (new_env_entry) {
                ...
                copy_env_var(new_env, env, i); // En caso de fallo de asignación
            }
            free(new_pwd);
        }
    } else {
        copy_env_var(new_env, env, i); // Copiar variables no modificadas
    }
}
...
void if_clean_complex_route(t_section *current) {
    ...
    if (!new_env) return; // Verificar error de asignación

    while (env[i]) {
        process_env_var(new_env, env, i);
        i++;
    }
    new_env[i] = NULL; // Terminar con NULL
