...
/*   Updated: 2024/11/08 21:38:37 by mgimon-c         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	child_process(t_section *current, int prev_fd, int *pipefd, int *statuspipe)
{
	char	notify;
	...
	if (ft_strcmp(current->cmdv[0], "cat") == 0)
		write(statuspipe[1], &notify, 1);
	close(statuspipe[1]);
...
void	parent_process(t_section **current, int *prev_fd, int *pipefd, int *statuspipe)
{
	char	buffer;
	...
	close(statuspipe[1]);
	if (read(statuspipe[0], &buffer, 1) > 0 && buffer == '1')
		Global_status = -1;
	close(statuspipe[0]);
...
void	executor(t_general *info)
{
	...
	while (current != NULL)
	{
		...
		if (current->pid == 0)
			handle_child_process(current, prev_fd, pipefd, statuspipe);
		else
			parent_process(&current, &prev_fd, pipefd, statuspipe);
	}
	wait_for_sections(info);
	Global_status = info->exit_status;
}
